...
import { getFrenchkissRabbitContract } from 'utils/contractHelpers'
import makeBatchRequest from 'utils/makeBatchRequest'

const frenchkissRabbitsContract = getFrenchkissRabbitContract()
...
const useGetWalletNfts = () => {
  ...
  useEffect(() => {
    const fetchNfts = async () => {
      try {
        const balanceOf = await frenchkissRabbitsContract.methods.balanceOf(account).call()

        if (balanceOf > 0) {
          const getTokenIdAndBunnyId = async (index: number) => {
            try {
              const { tokenOfOwnerByIndex, getBunnyId, tokenURI } = frenchkissRabbitsContract.methods
