...
const PageMeta = () => {
  ...
  const kissPriceUsdDisplay = kissPriceUsd.eq(0)
    ? ''
    : `$${kissPriceUsd.toNumber().toLocaleString(undefined, {
        minimumFractionDigits: 3,
        maximumFractionDigits: 3,
      })}`
  const pageMeta = customMeta[pathname] || {}
  const { title, description, image } = { ...DEFAULT_META, ...pageMeta }
  const pageTitle = kissPriceUsdDisplay ? [title, kissPriceUsdDisplay].join(' - ') : title
