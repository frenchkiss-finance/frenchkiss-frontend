...
import { useCompetitionKissRewards, getRewardGroupAchievements } from '../../helpers'
...
const UserPrizeGrid: React.FC<{ userTradingInformation?: UserTradingInformationProps }> = ({
  ...
  const { userRewardGroup, userKissRewards, userPointReward, canClaimNFT } = userTradingInformation
  const { kissReward, dollarValueOfKissReward } = useCompetitionKissRewards(userKissRewards)
  ...
  return (
    <StyledPrizeTable>
      <StyledThead>
        <tr>
          <th>{TranslateString(999, 'KISS Prizes ')}</th>
        ...
        <tr>
          <BoldTd>
            <Flex flexDirection="column">
              <Text bold>{kissReward.toFixed(2)}</Text>
              <Text fontSize="12px" color="textSubtle">
                ~{dollarValueOfKissReward} USD
              </Text>
            </Flex>
          </BoldTd>
          <Td>
