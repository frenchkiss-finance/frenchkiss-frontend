...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  const kissCosts = (amount: string): number => {
  ...
  return (
    <Modal title={TranslateString(450, 'Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        max={fullBalance}
        symbol="TICKET"
        availableSymbol="KISS"
      />
      <div>
        <Tips>{TranslateString(999, `1 Ticket = ${LOTTERY_TICKET_PRICE} KISS`, { num: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {TranslateString(
            478,
            'Ticket purchases are final. Your KISS cannot be returned to you after buying tickets.',
          )}
        </Announce>
        <Final>{TranslateString(460, `You will spend: ${kissCosts(val)} KISS`)}</Final>
      </div>
      <ModalActions>
