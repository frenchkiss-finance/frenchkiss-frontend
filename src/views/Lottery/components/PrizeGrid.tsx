...
import { Heading, Text } from '@frenchkiss-libs/uikit'
...
const PrizeGrid: React.FC<PrizeGridProps> = ({
  ...
  const kissBusdPrice = usePriceKissBusd()

  const getKissBusdValue = (amount: number) => {
    return new BigNumber(amount).multipliedBy(kissBusdPrice).toNumber()
  }
    ...
    <Grid pastDraw={pastDraw}>
      ...
      <GridItem>
        <RightAlignedHeading size="md">
          ...
          {!pastDraw && !kissBusdPrice.eq(0) && <CardBusdValue value={getKissBusdValue(fourMatchesAmount)} />}
        </RightAlignedHeading>
      </GridItem>
      {/* 3 matches row */}
      <GridItem>
      ...
      <GridItem>
        <RightAlignedText>
          ...
          {!pastDraw && !kissBusdPrice.eq(0) && <CardBusdValue value={getKissBusdValue(threeMatchesAmount)} />}
        </RightAlignedText>
      </GridItem>
      {/* 2 matches row */}
      <GridItem>
      ...
      <GridItem>
        <RightAlignedText>
          ...
          {!pastDraw && !kissBusdPrice.eq(0) && <CardBusdValue value={getKissBusdValue(twoMatchesAmount)} />}
        </RightAlignedText>
      </GridItem>
      {/* Burn row */}
