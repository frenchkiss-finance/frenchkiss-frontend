...
import { useKiss, useBunnyFactory } from 'hooks/useContract'
import useHasKissBalance from 'hooks/useHasKissBalance'
...
const minimumKissBalanceToMint = new BigNumber(MINT_COST).multipliedBy(new BigNumber(10).pow(18))

const Mint: React.FC = () => {
  const [bunnyId, setBunnyId] = useState(null)
  const { actions, minimumKissRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const kissContract = useKiss()
  const bunnyFactoryContract = useBunnyFactory()
  const TranslateString = useI18n()
  const hasMinimumKissRequired = useHasKissBalance(minimumKissBalanceToMint)
  const {
    isApproving,
    isApproved,
    isConfirmed,
    ...
    onRequiresApproval: async () => {
      ...
      try {
        const response = await kissContract.methods.allowance(account, bunnyFactoryContract.options.address).call()
  ...
  return (
    <>
      ...
      <Text as="p" mb="24px">
        {TranslateString(790, 'You can change your profile pic later if you get another approved FrenchKiss Collectible.')}
      </Text>
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {TranslateString(999, `Cost: ${MINT_COST} KISS`, { num: MINT_COST })}
          </Text>
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumKissRequired}
              >
                <Text bold>{nft.name}</Text>
              </SelectionCard>
            )
          })}
          {!hasMinimumKissRequired && (
            <Text color="failure" mb="16px">
              {TranslateString(1098, `A minimum of ${MINT_COST} KISS is required`)}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={bunnyId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumKissRequired}
