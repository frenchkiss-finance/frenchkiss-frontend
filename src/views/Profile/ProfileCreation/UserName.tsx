...
const minimumKissToRegister = new BigNumber(REGISTER_COST).multipliedBy(new BigNumber(10).pow(18))
...
const UserName: React.FC = () => {
  ...
  const { teamId, tokenId, userName, actions, minimumKissRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumKissRequired = useHasKissBalance(minimumKissToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumKissRequired={minimumKissRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumKissRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(1098, `A minimum of ${REGISTER_COST} KISS is required`, { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
