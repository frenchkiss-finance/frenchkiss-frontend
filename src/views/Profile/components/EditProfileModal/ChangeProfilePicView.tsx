...
import { Button, InjectedModalProps, Skeleton, Text } from '@frenchkiss-libs/uikit'
...
import { useFrenchkissRabbits, useProfile as useProfileContract } from 'hooks/useContract'
import { getFrenchkissProfileAddress, getFrenchkissRabbitsAddress } from 'utils/addressHelpers'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const {
    ...
    onApprove: () => {
      return frenchkissRabbitsContract.methods.approve(getFrenchkissProfileAddress(), tokenId).send({ from: account })
    },
    onConfirm: () => {
      if (!profile.isActive) {
        return profileContract.methods.reactivateProfile(getFrenchkissRabbitsAddress(), tokenId).send({ from: account })
      }

      return profileContract.methods.updateProfile(getFrenchkissRabbitsAddress(), tokenId).send({ from: account })
    },
    onSuccess: async () => {
      // Re-fetch profile
    ...
    <>
      ...
      {!isLoading && walletNfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {TranslateString(999, 'Make sure you have a FrenchKiss Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
