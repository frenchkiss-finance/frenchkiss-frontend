...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from '@frenchkiss-libs/uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const { numberKissToReactivate } = useGetProfileCosts()
  const TranslateString = useI18n()
  const frenchkissProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = () => {
    frenchkissProfileContract.methods
      .pauseProfile()
      .send({ from: account })
      .on('sending', () => {
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {TranslateString(999, `Cost to reactivate in future: ${getBalanceNumber(numberKissToReactivate)} KISS`)}
      </Text>
