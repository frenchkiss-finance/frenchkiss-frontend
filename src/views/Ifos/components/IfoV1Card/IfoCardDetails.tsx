...
const IfoCardDetails: React.FC<IfoCardDetailsProps> = ({ ifo, publicIfoData }) => {
  ...
  const { description, kissToBurn, projectSiteUrl, launchDate, launchTime, saleAmount, raiseAmount } = ifo
      ...
      {isOpen && (
        <>
          ...
          <Box mb="24px">
            ...
            <Item>
              <Display>{TranslateString(586, 'KISS to burn (USD)')}</Display>
              <Text>{kissToBurn}</Text>
            </Item>
            <Item>
