...
import { Heading, Card, CardBody, Button, useModal } from '@frenchkiss-libs/uikit'
...
import KissWinnings from './KissWinnings'
...
const LotteryCard = () => {
    ...
    if (!allowance.toNumber()) {
      return (
        <Button width="100%" disabled={requestedApproval} onClick={handleApprove}>
          {TranslateString(494, 'Approve KISS')}
        </Button>
      )
    }
    return (
  ...
  const [onPresentBuy] = useModal(<BuyModal max={kissBalance} tokenName="KISS" />)
  ...
  return (
    <StyledLotteryCard>
      <CardBody>
        ...
        <CardImage src="/images/ticket.svg" alt="kiss logo" width={64} height={64} />
        <Block>
          <Label>{TranslateString(552, 'KISS to Collect')}:</Label>
          <KissWinnings />
        </Block>
        <Block>
